    ...
    def readOpenTag(self, open_tag):
        ...
        if ':=' in open_tag:
            ...
            if self.tag in self.document.H_elements:
                ...
                if self.document.config['h1'] == True:
                    self.html += " id='section_%s' class='H_level_%s'" % (H_id_num_string, self.tag[1])
                else:
                    if self.tag != 'h1':
                        self.html += " id='section_%s' class='H_level_%s'" % (H_id_num_string, self.tag[1])
  ...
  window.WebFontConfig = {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if self.config['highlight']:
            ...
            html += '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.4.0/styles/default.min.css">\n'
            ...
            for h in self.toc_list:
                ...
                if h.startswith('<h1') and self.config['h1'] == False:
                    ...
                    if int(tag[1]) <= self.config['toc']:
                        hlink = h.replace("<%s id='" % tag, "<div><a href='#" )
...
def run():
    ...
    config = {'sourcefile':None, 'outputfile':None, 'toc':2, 'h1':0, 'numbers': 0, 'includes': 4 ,'css':'default', 'katex':0, 'highlight': 0, 'x3d':0, 'dir':'ltr', 'lang':'en', 'class': 'hh', 'view': False}
